...
import { buildReports, reportRows, rowsToCsv, type ReportBundle, type ReportPeriod } from './report-utils';
...
export function ReportsScreen({ onNavigate }: { onNavigate?: (section: string) => void }) {
    ...
    try {
      ...
      if (format === 'excel') {
        if (Platform.OS === 'web') {
          ...
          if (await Sharing.isAvailableAsync()) await Sharing.shareAsync(uri, { mimeType: 'text/csv', dialogTitle: 'Export Khata report to Excel' });
        }
        return;
      }
      ...
      if (Platform.OS === 'web') {
        ...
        if (!popup) throw new Error('Allow pop-ups to print this report as PDF.');
        ...
        if (await Sharing.isAvailableAsync()) await Sharing.shareAsync(result.uri, { mimeType: 'application/pdf', UTI: 'com.adobe.pdf', dialogTitle: 'Export Khata report as PDF' });
      }
    } finally {
      setExporting(null);
    }
  ...
  return <Screen>
    <View style={styles.header}><View style={styles.headerCopy}><Eyebrow>{company.name} · {bundle.periodLabel}</Eyebrow><Title subtitle="Accounting views built from the same workspace records.">Reports</Title></View><Chip tone="green">Live workspace data</Chip></View>
    <Card style={styles.hero}><View style={styles.heroCopy}><Chip tone="gold">{selected.title}</Chip><Text style={styles.heroTitle}>{heroValue(kind, bundle)}</Text><Text style={styles.heroText}>{selected.description}. Change the period or export exactly this view.</Text></View><View style={styles.heroAside}><Text style={styles.heroAsideLabel}>Entries</Text><Text style={styles.heroAsideValue}>{bundle.dayBook.length}</Text><Text style={styles.heroAsideHint}>posted journal lines</Text></View></Card>
    ...
    <View style={styles.reportTabs}>{reportKinds.map(item => { const Icon = item.icon; return <Pressable key={item.id} onPress={() => setKind(item.id)} style={[styles.reportTab, kind === item.id && styles.reportTabActive]}><View style={[styles.reportIcon, kind === item.id && styles.reportIconActive]}><Icon size={18} color={kind === item.id ? C.white : C.blue} /></View><View style={styles.reportTabCopy}><Text style={[styles.reportTabTitle, kind === item.id && styles.reportTabTitleActive]}>{item.title}</Text><Text style={[styles.reportTabDetail, kind === item.id && styles.reportTabDetailActive]}>{item.description}</Text></View></Pressable>; })}</View>

    <ReportHighlights bundle={bundle} kind={kind} />
    <View style={styles.tableHeader}><View style={styles.tableTitle}><Text style={styles.tableTitleText}>{selected.title}</Text><Text style={styles.tableDetail}>{rows.length} rows · {bundle.periodLabel}</Text></View><View style={styles.exportActions}><Button label={exporting === 'pdf' ? 'Preparing…' : 'PDF'} variant="outline" icon={<DownloadIcon size={14} color={C.brick} />} onPress={() => { void exportReport('pdf'); }} disabled={Boolean(exporting)} /><Button label={exporting === 'excel' ? 'Preparing…' : 'Excel'} icon={<DownloadIcon size={14} color={C.white} />} onPress={() => { void exportReport('excel'); }} disabled={Boolean(exporting)} /></View></View>
    <ScrollView horizontal showsHorizontalScrollIndicator={false}><Card style={styles.tableCard}>{rows.length ? <><View style={styles.rowHeader}>{columns.map(column => <Text key={column} style={[styles.cell, styles.headerCell]}>{column}</Text>)}</View>{rows.map((row, index) => <View key={`${kind}-${index}`} style={styles.dataRow}>{columns.map(column => <Text key={column} style={styles.cell} numberOfLines={2}>{formatCell(row[column], column)}</Text>)}</View>)}</> : <View style={styles.noData}><Text style={styles.noDataTitle}>No entries for this period</Text><Text style={styles.noDataText}>Add a sale, purchase or expense to populate this report.</Text></View>}</Card></ScrollView>
    <Text style={styles.disclaimer}>Reports are calculated from normalized workspace records currently synced to Supabase. VAT uses each purchase bill’s VAT amount and the workspace’s default 13% sales rate until per-sale VAT lines are recorded.</Text>
  </Screen>;
}

function heroValue(kind: ReportKind, bundle: ReportBundle) {
  if (kind === 'profit-loss') return money(bundle.profitLoss.netProfit);
  if (kind === 'balance-sheet') return money(bundle.balanceSheet.totalAssets);
  if (kind === 'vat-summary') return money(bundle.vatSummary.netVat);
  if (kind === 'stock-report') return money(bundle.stockReport.reduce((sum, row) => sum + row.value, 0));
  if (kind === 'trial-balance') return `${money(bundle.trialBalance.totalDebit)} balanced`;
  return `${bundle.dayBook.length} journal lines`;
}

function ReportHighlights({ bundle, kind }: { bundle: ReportBundle; kind: ReportKind }) {
  if (kind === 'profit-loss') return <View style={styles.highlights}>{[['Revenue', bundle.profitLoss.revenue], ['Gross profit', bundle.profitLoss.grossProfit], ['Net profit', bundle.profitLoss.netProfit]].map(([label, value]) => <MiniMetric key={String(label)} label={String(label)} value={money(Number(value))} />)}</View>;
  if (kind === 'balance-sheet') return <View style={styles.highlights}>{[['Assets', bundle.balanceSheet.totalAssets], ['Liabilities', bundle.balanceSheet.liabilities.reduce((sum, row) => sum + row.amount, 0)], ['Equity', bundle.balanceSheet.equity]].map(([label, value]) => <MiniMetric key={String(label)} label={String(label)} value={money(Number(value))} />)}</View>;
  if (kind === 'vat-summary') return <View style={styles.highlights}>{[['Output VAT', bundle.vatSummary.outputVat], ['Input VAT', bundle.vatSummary.inputVat], ['Net payable / credit', bundle.vatSummary.netVat]].map(([label, value]) => <MiniMetric key={String(label)} label={String(label)} value={money(Number(value))} />)}</View>;
  ...
  return <View style={styles.highlights}><MiniMetric label="Debit total" value={money(bundle.trialBalance.totalDebit)} /><MiniMetric label="Credit total" value={money(bundle.trialBalance.totalCredit)} /><MiniMetric label="Day book lines" value={String(bundle.dayBook.length)} /></View>;
}

function MiniMetric({ label, value }: { label: string; value: string }) { return <View style={styles.miniMetric}><Text style={styles.miniLabel}>{label}</Text><Text style={styles.miniValue}>{value}</Text></View>; }
function formatCell(value: string | number | undefined, column: string) { if (typeof value === 'number' && (column === 'Amount' || column === 'Debit' || column === 'Credit' || column.includes('value') || column.includes('VAT') || column === 'Margin')) return money(value); return String(value ?? '—'); }
...
const styles = StyleSheet.create({
  ...
  hero: { flexDirection: 'row', alignItems: 'stretch', gap: 18, backgroundColor: C.brickDark, borderColor: C.brickDark },
  heroCopy: { flex: 1, minWidth: 220, gap: 10 },
  heroTitle: { color: C.white, fontSize: 30, lineHeight: 34, fontWeight: '900' },
  heroText: { color: 'rgba(255,255,255,0.72)', fontSize: 13, lineHeight: 20 },
  heroAside: { minWidth: 130, justifyContent: 'flex-end', padding: 14, backgroundColor: C.orange },
  ...
  periodActive: { backgroundColor: C.blue, borderColor: C.blue },
  ...
  reportTabActive: { backgroundColor: C.ink, borderColor: C.ink },
  reportIcon: { width: 34, height: 34, alignItems: 'center', justifyContent: 'center', backgroundColor: '#E7ECFF' },
  reportIconActive: { backgroundColor: C.blue },
  ...
  miniMetric: { flex: 1, minWidth: 145, padding: 14, backgroundColor: C.bone, borderLeftColor: C.orange, borderLeftWidth: 3 },
